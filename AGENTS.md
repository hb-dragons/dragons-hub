...
| `probetrainingSubmissions` | `packages/db/src/schema/probetraining-submissions.ts` | month (German name), year, didPlay, gender, mail, message, acceptedPrivacy — legacy-form field names 1:1; deliberately **no IP column** (GDPR minimization — the rate limiter keeps IPs only in Redis with a 1 h TTL) |
...
| Method | Path | Description |
|--------|------|-------------|
| GET | `/admin/matches` | List own club matches (with booking info); `includeGhosts=true` adds the game plan's ghost entries (`kind: "ghost"`, not counted in `total`) |
...
| POST | `/public/probetraining` | Probetraining (trial training) submission from the club website (no auth). Body: legacy-form fields `{ month, year, didPlay, gender, mail, message?, acceptedPrivacy, website }`; `website` is a honeypot — any content is silently dropped with a fake 201. Rate limit 5/hour/IP (fixed window in Redis, IP never stored — GDPR); 429 beyond. Stores the row and mails `PROBETRAINING_NOTIFY_TO` (reply-to = submitter); mail failure still answers 201. |
