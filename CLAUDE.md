...
GOOGLE_GENERATIVE_AI_API_KEY=<google ai studio key; required when CHATBOT_ENABLED is true>
```
...
The club Q&A chatbot feature vars are threaded from GitHub through TF (set the corresponding GitHub Actions secret/variable, then `opentofu.yml` passes it as `TF_VAR_*`):
- **Feature flag** `CHATBOT_ENABLED`: GitHub repository variable → `TF_VAR_chatbot_enabled` (empty `|| 'false'` fallback) → API/Worker `env_vars`. It also feeds the web build-arg `NEXT_PUBLIC_CHATBOT_ENABLED` in `deploy.yml` from the same variable, so the frontend entry point and the backend stay in sync from one source (the web `ARG`/`ENV` pair lives in `apps/web/Dockerfile`). The model (`CHATBOT_MODEL`) uses its TF default; not passed from the workflow.
- **Secret** `GOOGLE_GENERATIVE_AI_API_KEY` (required when the flag is `true` — the API env schema rejects boot otherwise): GitHub Actions secret → `TF_VAR_*` → Secret Manager (`google-generative-ai-api-key-production`) → mounted via `secrets` on both API + Worker (both run the same env schema).
