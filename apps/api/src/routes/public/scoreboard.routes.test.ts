...
vi.mock("../../services/scoreboard/live-snapshot", () => ({
  getLatestSnapshot: (...a: unknown[]) => mocks.getLatestSnapshot(...a),
}));
...
describe("GET /public/scoreboard/latest", () => {
  ...
  it("returns 500 through the shared error handler when the service throws", async () => {
    mocks.getLatestSnapshot.mockRejectedValue(new Error("db exploded"));
    const res = await app.request("/public/scoreboard/latest?deviceId=d1");
...
describe("GET /public/scoreboard/stream", () => {
  it("delegates to createScoreboardStream", async () => {
    ...
    const r = await app.request("/public/scoreboard/stream?deviceId=d1", {
  ...
  it("rejects a missing deviceId with the shared envelope", async () => {
    const res = await app.request("/public/scoreboard/stream");
    expect(res.status).toBe(400);
    expect(await res.json()).toMatchObject({
      code: "VALIDATION_ERROR",
      details: expect.any(Array),
