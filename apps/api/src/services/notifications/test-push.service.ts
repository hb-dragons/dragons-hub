...
import { channelConfigs, notificationLog, pushDevices } from "@dragons/db/schema";
...
export interface TestPushTicketResult {
  platform: string;
  status: string;
...
export interface TestPushRecentResult {
  id: number;
  sentAt: Date;
...
/**
 * Sends a test push to every device the calling admin has registered, and
 * records the send as one aggregate `notification_log` row.
 */
export async function sendAdminTestPush(input: {
  callerId: string;
  message?: string;
}): Promise<{ deviceCount: number; tickets: TestPushTicketResult[] }> {
  const { callerId, message } = input;
  ...
  if (devices.length === 0) {
    throw new TestPushError(
      "Open the native app on a signed-in device first.",
      "NO_DEVICES",
    );
  }
  ...
  if (!pushChannel) {
    throw new TestPushError(
      "No push channel is configured",
      "PUSH_CHANNEL_MISSING",
    );
  }

  const sentAt = new Date();
  const eventId = `admin_test:${callerId}:${ulid()}`;
  const text = message ?? "Test push from Dragons admin";
  ...
  try {
    ...
    tickets = devices.map(() => ({ status: "error" as const, message: errMessage }));
  }
...
/** The calling admin's own recent test-push results, newest first. */
export async function listRecentTestPushes(
  callerId: string,
): Promise<TestPushRecentResult[]> {
