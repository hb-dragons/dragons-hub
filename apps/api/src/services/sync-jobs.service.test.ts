...
describe("initializeScheduledJobs", () => {
  ...
  it("uses defaults on DB read error", async () => {
    ...
    // Referee schedule falls back to the 30-minute default
    expect(mockAdd).toHaveBeenCalledWith(
      "referee-games-sync-scheduled",
      { type: "referee-games" },
...
describe("triggerManualSync", () => {
  ...
  it("throws SyncAlreadyQueuedError when one is queued", async () => {
    mockGetJob.mockResolvedValue({ getState: vi.fn().mockResolvedValue("waiting") });
    mockGetJobs.mockResolvedValue([]);

    await expect(triggerManualSync()).rejects.toThrow(SyncAlreadyQueuedError);
    expect(mockAdd).not.toHaveBeenCalled();
    expect(mockInsert).not.toHaveBeenCalled();
  });

  it("carries the SYNC_ALREADY_QUEUED code and message on the thrown error", async () => {
    mockGetJob.mockResolvedValue({ getState: vi.fn().mockResolvedValue("active") });
    mockGetJobs.mockResolvedValue([]);

    const error = await triggerManualSync().catch((err: unknown) => err);

    expect(error).toBeInstanceOf(SyncAlreadyQueuedError);
    expect(error).toMatchObject({
      name: "SyncAlreadyQueuedError",
      code: "SYNC_ALREADY_QUEUED",
      message: "Sync already in progress or queued",
    });
  });

  it("releases the sync lock when it rejects", async () => {
  ...
  it("throws when daily-sync is running", async () => {
    mockGetJob.mockResolvedValue(null);
  ...
  it("does not write an orphan pending run when the lock is held (#71)", async () => {
    ...
    await expect(triggerManualSync("user-1")).rejects.toThrow(SyncAlreadyQueuedError);

    expect(mockSet).toHaveBeenCalledWith("lock:sync:full", expect.anything(), "EX", expect.any(Number), "NX");
    expect(mockInsert).not.toHaveBeenCalled();
    expect(mockAdd).not.toHaveBeenCalled();
    // The lock was never acquired, so it must not be deleted either.
  ...
  it("logs a warning but does not reject when releasing the lock fails", async () => {
    mockGetJob.mockResolvedValue(null);
    mockGetJobs.mockResolvedValue([]);
    mockDel.mockRejectedValue(new Error("redis down"));

    await expect(triggerManualSync("user-1")).resolves.toMatchObject({ status: "queued" });
  });
});
