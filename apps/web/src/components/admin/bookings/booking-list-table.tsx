...
export function BookingListTable() {
  ...
  function formatTimeWindow(booking: BookingListItem): string {
    ...
    const start = format.dateTime(clubTimeAnchor(booking.effectiveStartTime, booking.date), "matchTime");
    const end = format.dateTime(clubTimeAnchor(booking.effectiveEndTime, booking.date), "matchTime");
    return `${start} \u2013 ${end}`;
  }
