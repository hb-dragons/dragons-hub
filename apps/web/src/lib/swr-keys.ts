...
export const SWR_KEYS = {
  ...
  // The admin game plan (ghost entries included). Season-scoped variant: the
  // key carries no seasonId when no season is named, so the server prefetch
  // (which lets the API default to the active season) and the client's initial
  // render share one cache entry instead of racing to fetch the same list twice.
  matches: (seasonId?: number) =>
    seasonId === undefined
      ? "/admin/matches?includeGhosts=true"
      : `/admin/matches?includeGhosts=true&seasonId=${seasonId}`,
