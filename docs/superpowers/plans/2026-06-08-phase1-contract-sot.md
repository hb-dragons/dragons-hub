...
> **MANDATORY NAMING CONVENTION (set by the tracer's final review — apply from the first group):** Every export in `@dragons/contracts` MUST be prefixed with its domain noun so the barrel never collides as 25 modules land. The board schemas already follow this (`boardIdParamSchema`, `columnReorderBodySchema`). Several existing API `.schemas.ts` files use **bare generic names that WILL collide** and must be renamed on the way in — e.g. `idParamSchema` (social) → `socialIdParamSchema`; `paginationSchema` (sync) → `syncPaginationSchema`; `ruleItemSchema`/`rulesArraySchema` (referee-rules) → `refereeRule*`; generic `create*/update*ChannelConfigSchema`, `create*/update*WatchRuleSchema` are fine (already prefixed). When you rename an export, update its importers in the route file in the same step. Because of this, **`index.ts` uses explicit named re-exports per module, NOT `export *`** — a named re-export surfaces a collision as an immediate, localized error instead of a confusing `export *` ambiguity.

- [ ] **Step 1:** Create `packages/contracts/src/<G>.ts` containing the group's request schemas. If the API already has `apps/api/src/routes/<area>/<G>.schemas.ts`, move its contents and add `z.infer` type exports for every body schema. **Apply the naming convention above**: any bare/generic export name gets a domain-noun prefix (and its route-file importers updated to match). If the schemas are **inline `z.object(...)`** in the route file, extract them into `packages/contracts/src/<G>.ts` first, already prefixed.
- [ ] **Step 2:** Re-export the group's symbols by name from `packages/contracts/src/index.ts` (e.g. `export { matchCreateBodySchema, type MatchCreateBody, ... } from "./match";`) — **not** `export *`. After adding, run `pnpm --filter @dragons/contracts typecheck` to catch any name collision immediately.
...
## Follow-ups surfaced by the tracer (carry into the remaining groups / Phase 2-3)

- **Convert the board barrel to named re-exports.** `packages/contracts/src/index.ts` currently has `export * from "./board"` (fine while it's the only module). When the first additional group is added, switch it to explicit named re-exports too, so the whole barrel is consistent and collision-safe.
- **Re-export `ColumnReorderBody` through api-client** (or have the reorder call sites import it). It's exported by `@dragons/contracts` but not surfaced via `@dragons/api-client`; native/web reorder callers still use an inline `{ id; position }[]`. One-line add to `packages/api-client/src/endpoints/index.ts` + the top-level barrel.
- **api-client coverage ratchet (Phase 2).** The Phase 1 baseline gate sits at the measured floor (functions 78%). Untested api-client methods to cover: `admin-board.ts` `updateColumn`, `deleteColumn`, `getTask`, `updateTask`, `updateChecklistItem`, `deleteChecklistItem`, `deleteComment`; and `devices.ts` (`register`, `unregister`) has no test file. Raise the thresholds toward 90/95 as these land.
- **`validation.test.ts` style nit (trivial):** the object-path branch test uses `.at(0)?.` where the rest of the file uses `[0]!`; align if touched.
- **`apps/web` board mutation hooks are not on the contract path.** `use-board-mutations.ts` / `use-column-mutations.ts` declare their own `*Input` interfaces and call `fetchAPI` directly — structurally identical to the contracts types today but unguarded against drift. Closing this is part of the **Phase 3** web→api-client migration (already out of scope below); listed here so the drift risk is explicit.
