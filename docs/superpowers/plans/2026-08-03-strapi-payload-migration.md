...
- The migration script **is** covered. `coverage.include` was widened to `scripts/**/*.ts` on 2026-08-10, and the thresholds were re-measured against that ~3x larger scope: statements and lines ratcheted up (98 -> 99), branches (90) and functions (98) re-measured lower. That is a re-measurement, not a relaxed gate — do not "restore" the older numbers. `index.ts` is the one exclusion, so importing `main` in a test cannot start a migration against whatever `CMS_URL` happens to be set.
